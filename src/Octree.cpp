...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	...
	for (int i = 0; i < childBoxes.size(); i++) {
		...
		vector<int> points;
		int pointCount = getMeshPointsInBox(mesh, node.points, childBoxes[i], points);

		if (pointCount > 0) {
			childNode.points = points;
			node.children.push_back(childNode);
			int size = node.children.size() - 1;

			subdivide(mesh, node.children[size], numLevels, level + 1);
...
void Octree::draw(TreeNode & node, int numLevels, int level) {
	...
	// Color
	ofColor color;
	color.setHsb((level*40) % 255, 255, 255);
	ofSetColor(color);
	...
	for (int i = 0; i < node.children.size(); i++) {
		draw(node.children[i], numLevels, nextLevel);
	}
}
